...
class StorageManager:
    ...
    def save_splits(self, train_ds, dev_ds, test_ds, train_raw=None, dev_ratio: float = 0.20, seed: int = 42, dataset_name: str = "ailsntua/QEvasion", task: str = "clarity"):
        ...
        with open(splits_path, 'wb') as f:
            pickle.dump({
                ...
                # Save actual dataset objects to avoid HuggingFace dependency
                'train_ds': train_ds,
                'dev_ds': dev_ds,
                'test_ds': test_ds
            }, f)
    ...
    def load_split(self, split_name: str, task: str = 'clarity'):
        ...
        # NEW: If dataset objects are saved in pickle, use them directly (no HuggingFace needed)
        if split_name == 'train' and 'train_ds' in splits_data:
            return splits_data['train_ds']
        elif split_name == 'dev' and 'dev_ds' in splits_data:
            return splits_data['dev_ds']
        elif split_name == 'test' and 'test_ds' in splits_data:
            return splits_data['test_ds']
        
        # BACKWARD COMPATIBILITY: If datasets not in pickle, load from HuggingFace (old format)
        # This handles old pickle files that only contain indices
        dataset_name = splits_data.get('dataset_name', 'ailsntua/QEvasion')
        is_filtered = splits_data.get('is_filtered', False)
        
        # Reload dataset from HuggingFace (fallback for old format)
        try:
            dataset = load_dataset(dataset_name)
        except Exception as e:
            raise RuntimeError(
                f"Failed to load dataset from HuggingFace: {e}\n"
                f"This is likely a timeout or network issue.\n"
                f"Please re-run 01_data_split.ipynb to save dataset objects in pickle file."
            ) from e
